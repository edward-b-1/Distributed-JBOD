...
- Writes place around devices the cluster cannot read: placement leaves an unavailable device out as it leaves out a full one, the write goes ahead if k+m devices with room remain, and its response names the devices it went around; a write is one placement and one attempt, never retried on another device (SPEC 5.6, 10.7) (#186).
- No write can recreate a device's tree: a write makes the partition and key directories with a plain `mkdir` and treats a missing parent as the device being unavailable, so a repair no longer rebuilds a dead disk's directory on whatever filesystem sits at its path (SPEC 5.6) (#185).
