...
use djbod_core::erasure::{Coder, CodingError, Scheme, ShardIndex};
...
fn coder_for(k: u8, m: u8) -> Coder {
    Coder::new(Scheme::new(k, m).expect("failed to construct scheme"))
}
...
fn full_stripe_splits_contiguously_and_data_blocks_are_verbatim() {
    for &(k, m) in SCHEMES {
        let coder = coder_for(k, m);
        let data = xorshift64_bytes(k as usize * BLOCK_SIZE, 1);
        let encoded = encode_stripe(&coder, &data, BLOCK_SIZE).expect("failed to encode stripe");
...
fn short_final_stripe_is_padded_to_equal_blocks() {
    ...
    let coder = coder_for(3, 1);
    let block_size = 1 << 20;
    let data = xorshift64_bytes(1 << 20, 2);
    let encoded = encode_stripe(&coder, &data, block_size).expect("failed to encode stripe");
    assert_eq!(encoded.block_len(), 349_526);
    assert_eq!(block_length_for(&coder, data.len()), 349_526);
    ...
    let decoded = decode_stripe(&coder, &all_received(&encoded), data.len())
...
fn stripes_shorter_than_k_bytes_still_encode() {
    ...
    for len in 1..=5 {
        ...
        let decoded =
            decode_stripe(&coder, &all_received(&encoded), len).expect("failed to decode stripe");
...
fn empty_and_oversized_stripes_are_rejected() {
    let coder = coder_for(3, 1);
    assert_eq!(
        encode_stripe(&coder, &[], BLOCK_SIZE),
        Err(StripeError::Empty)
    );
    let too_big = vec![0u8; 3 * BLOCK_SIZE + 1];
    assert_eq!(
        encode_stripe(&coder, &too_big, BLOCK_SIZE),
        Err(StripeError::TooLarge {
            actual: 3 * BLOCK_SIZE + 1,
            max: 3 * BLOCK_SIZE
        })
    );
    assert_eq!(decode_stripe(&coder, &[], 0), Err(StripeError::Empty));
}
...
fn intact_blocks_decode_with_no_faults_and_no_parity_needed() {
    for &(k, m) in SCHEMES {
        ...
        let decoded =
            decode_stripe(&coder, &received, data.len()).expect("failed to decode stripe");
        ...
        for fault in &decoded.faults {
            assert!(coder.scheme().is_parity(fault.index));
...
fn a_flipped_bit_is_reported_as_a_checksum_mismatch_and_repaired() {
    for &(k, m) in SCHEMES {
        ...
        for victim in 0..(k + m) as usize {
            ...
            let decoded = decode_stripe(&coder, &received, data.len())
...
fn every_damage_pattern_up_to_m_is_repaired_and_reported_exactly() {
    ...
    for &(k, m) in SCHEMES {
        ...
        let encoded = encode_stripe(&coder, &data, 256).expect("failed to encode stripe");
        ...
        for damaged in 1..=m as usize {
            for pattern in combinations(n, damaged) {
                ...
                let decoded = decode_stripe(&coder, &kept, data.len())
...
fn more_than_m_damaged_blocks_is_an_error_listing_every_fault() {
    for &(k, m) in SCHEMES {
        ...
        let encoded = encode_stripe(&coder, &data, 128).expect("failed to encode stripe");
        ...
        let err = decode_stripe(&coder, &received, data.len()).expect_err("decode should fail");
        match err {
...
fn swapped_blocks_are_both_caught_by_their_checksums() {
    ...
    let encoded = encode_stripe(&coder, &data, 512).expect("failed to encode stripe");
    let mut received = all_received(&encoded);
    ...
    let decoded = decode_stripe(&coder, &received, data.len()).expect("failed to decode stripe");
    assert_eq!(decoded.data, data);
...
fn no_parity_scheme_round_trips_and_cannot_repair() {
    let coder = coder_for(3, 0);
    let data = xorshift64_bytes(3 * 1000, 9);
    let encoded = encode_stripe(&coder, &data, 1000).expect("failed to encode stripe");
...
fn a_wrong_stored_checksum_condemns_a_good_block() {
    ...
    let encoded = encode_stripe(&coder, &data, 64).expect("failed to encode stripe");
    let mut received = all_received(&encoded);
...
fn protocol_errors_are_distinguished_from_faults() {
    let coder = coder_for(2, 1);
    let data = xorshift64_bytes(2 * 32, 11);
    let encoded = encode_stripe(&coder, &data, 32).expect("failed to encode stripe");

    let mut duplicate = all_received(&encoded);
    duplicate.push(duplicate[0].clone());
    assert_eq!(
        decode_stripe(&coder, &duplicate, data.len()),
    ...
    assert_eq!(
        decode_stripe(&coder, &out_of_range, data.len()),
...
fn fault_list_is_in_shard_index_order_regardless_of_arrival_order() {
    ...
    let encoded = encode_stripe(&coder, &data, 16).expect("failed to encode stripe");
    let mut received = all_received(&encoded);
    received.reverse();
