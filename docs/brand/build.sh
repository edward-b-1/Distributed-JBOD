...
#   OUT=status-light ORANGE='#FFD21A' GLOW=1 bash build.sh # lit status lights
set -e
IK="${INKSCAPE:-/c/Program Files/Inkscape/bin/inkscape.exe}"
BLUE="${BLUE:-#2a78d6}";   ORANGE="${ORANGE:-#F97316}"
BLUE_D="${BLUE_D:-#3987e5}"
# GLOW=1 lights the parity slabs (see gen.awk); unset leaves them flat.
LIT="${GLOW:+glow}"
...
mark () { # $1=out $2=blue $3=orange
  awk -f "$GEN" -e "BEGIN{
    ...
    printf \"%s\", stack(4,44,23,10,8,\"4 6 3 5\",\"3 2 1 0\",\"$2\",\"$3\",256,\"$LIT\");
    printf \"</svg>\n\";
  }" > "$1"
}
...
awk -f "$GEN" -e "BEGIN{
  printf \"<svg xmlns=\\\"http://www.w3.org/2000/svg\\\" width=\\\"256\\\" height=\\\"256\\\" viewBox=\\\"0 0 256 256\\\">\n  <title>Distributed JBOD</title>\n\";
  printf \"%s\", stack(3,56,40,18,14,\"3 2 4\",\"2 1 0\",\"$BLUE\",\"$ORANGE\",256,\"$LIT\");
  printf \"</svg>\n\";
}" > djbod-icon-small.svg
awk -f "$GEN" -e "BEGIN{
  printf \"<svg xmlns=\\\"http://www.w3.org/2000/svg\\\" width=\\\"256\\\" height=\\\"256\\\" viewBox=\\\"0 0 256 256\\\">\n  <title>Distributed JBOD</title>\n\";
  printf \"%s\", stack(2,96,66,24,24,\"2 2\",\"1 0\",\"$BLUE\",\"$ORANGE\",256,\"$LIT\");
  printf \"</svg>\n\";
}" > djbod-favicon.svg
awk -f "$GEN" -e "BEGIN{
  ...
  printf \"%s\", stack(3,56,40,18,14,\"3 2 4\",\"2 1 0\",\"$BLUE_D\",\"$ORANGE\",256,\"$LIT\");
  printf \"  </g>\n</svg>\n\";
}" > djbod-appicon.svg
...
lockup_h () { awk -f "$GEN" -e "BEGIN{
  ...
  printf \"%s\", stack(4,44,23,10,8,\"4 6 3 5\",\"3 2 1 0\",\"$BLUE\",\"$ORANGE\",256,\"$LIT\");
...
if [ "$OUT" = "." ]; then png proof-palette.svg proof-palette.png 1020; fi
